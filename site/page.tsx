...
import pages, { ComponentGroup } from './pages';
...
interface PageState {locale?:Lang,page: string}
...
export default class App extends React.Component<PageProps,PageState> {
  ...
  componentDidUpdate(props: PageProps, state: PageState) {
  ...
  getLocale(key: string) {
    const map = locales[this.state.locale] || {} as any;
  ...
  getPage() {
    ...
    if (routes) {
      if (locales.hasOwnProperty(routes[1])) {
        this.setState({ locale: routes[1] as Lang }, () => {
          ...
          <nav className="side-nav">
            <ul>
              ...
              <li className="nav-item">
                ...
                {
                  Object.keys(pages.components).map((group: ComponentGroup) => {
                    return (
