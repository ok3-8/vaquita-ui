...
export default class Canvas extends React.Component<{locale: any,name: string},CanvasState> {
  static defaultProps = {
    locale: {}
  }
  ...
  constructor(props: any) {
    super(props)
    this.playerId = `${(Math.random() * 1e9).toString(36)}`
    this.document = (this.props.children as any).match(/([^]*)\n?(```[^]+```)/)
        ...
        <div className="source" id={this.playerId} />
        {
          this.state.showBlock && (
            <div className="meta">
              ...
              <Editor
                value={this.source[2]}
                onChange={(code:string) => this.renderSource(code)}
              />
            </div>
          )
        }
